...
Префільтр — це функція, яка буде викликана до події `ajaxStart`, в ній ти зможеш змінити як об'єкт `jqXHR`, так і будь-які супутні налаштування:
...
Префільтри можна «навішувати» на певний тип `dataType` (тобто в залежності від очікуваного типу даних від сервера будуть спрацьовувати різні фільтри):
...
Ну і останнє: для перемикання `dataType` на якийсь інший тип нам достатньо буде повернути необхідне значення:
...
{% hint style="warning" %}
Будь дуже обережний, коли оперуєш глобальними налаштуваннями, та ще через таку неявну фічу, як фільтри. Задокументуй подібні підходи в супровідній документації, інакше розробники, які будуть надалі супроводжувати твій код, будуть сильно лаятися (як такі через пару місяців можеш виявитися і ти сам). І загалом настійно рекомендується достатньо детальне документування та коментування коду. Влучна фраза, помилково приписувана метру програмування МакКоннеллу: _"Пишіть код так, ніби супроводжувати його буде схильний до насильства психопат, який знає, де ви живете"_ (Джон Ф. Вудс).
{% endhint %}

## Конвертори <a href="#converters" id="converters"></a>

Конвертор — функція зворотного виклику, яка викликається в тому випадку, коли отриманий тип даних не збігається з очікуваним (тобто `dataType` вказано невірно).

Всі конвертори зберігаються в глобальних налаштуваннях `ajaxSettings`:
...
> Трішки пояснень: ми запитуємо «XML», який конвертуємо в текст, який буде переданий в наш конвертор з «`text`» в «`mydatatype`».

## Транспорт <a href="#transport" id="transport"></a>

{% hint style="warning" %}
Використання свого транспорту — це крайній захід, вдавайся до нього лише в тому випадку, якщо з поставленим завданням не можна впоратися з використанням префільтрів та конверторів.
{% endhint %}
...
А тепер мега-напруга — додамо транспорт «`image`» на нашу сторінку:
...
> Я хотів би ще раз нагадати, що це «advanced level», і даний розділ зайвий в підручнику «для початківців».

По слідах офіційної документації:
