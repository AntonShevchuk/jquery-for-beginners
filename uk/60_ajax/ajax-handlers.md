...
Для зручності розробки AJAX-запити кидають кілька подій, і їх, природно, можна та потрібно обробляти. jQuery дозволяє обробляти ці події для кожного AJAX-запиту й окремо та глобально. Наведу схемку, на якій наочно видно порядок виникнення подій в jQuery:
...
{% hint style="warning" %}
Це задачка з юзабіліті — ми завжди повинні тримати користувача сайту в курсі справи про те, що відбувається на сторінці та відправлення AJAX-запиту теж потрапляє під розряд «must know». Подібне рішення у вас буде практично на будь-якому сайті, де ходить AJAX.
{% endhint %}

Для локальних подій — уносимо зміни в опції методу `ajax()`:
