...
Подивіться, як працює система без нашого втручання — клацніть чекбокс, селектбокс, спробуйте відправити форму.

Тепер розпочнемо серію експериментів (не забудьте оновити сторінку):
...
{% hint style="info" %}
Невелике пояснення суті того, що відбувається. При першому виклику методу `attr("checked", "checked")` проставляється галочка, оскільки змінюються й атрибут та властивість `checked`. При повторному виклику вже нічого не відбувається, змінюється лише значення атрибута, яке й так вже `checked`.
{% endhint %}
...
Ну й ще один експеримент з другим чекбоксом:
