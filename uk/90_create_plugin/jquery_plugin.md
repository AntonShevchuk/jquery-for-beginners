...
Для початку згадаймо, для чого нам потрібні плагіни? Моя відповідь — для створення повторно використовуваного коду, причому зі зручним інтерфейсом. Напишімо такий код, почнемо з простого завдання:
...
Щоб не забувати джерела, почнемо з реалізації на нативному JavaScript:
...
{% hint style="info" %}
Внесу невелике пояснення про «магію», що тут відбувається. Код `(function($){…})(jQuery)` створює анонімну функцію і тут же викликає її, передаючи як параметр об'єкт `jQuery`. Таким чином усередині анонімної функції ми можемо використовувати аліас `$`, не боячись конфліктів з іншими бібліотеками, оскільки тепер `$` знаходиться лише в області видимості нашої функції та ми маємо повний контроль над нею. Якщо у вас виникло відчуття дежавю – то все правильно, я про це вже розповідав.
{% endhint %}
...
У результаті роботи цього плагіна кожен клік змінюватиме колір параграфа на червоний. Оскільки ми використовуємо глобальну змінну для зберігання налаштувань, другий виклик плагіна змінить значення для всіх елементів:

{% embed url="https://anton.shevchuk.name/book/code/plugin.global.html" %}

Можна внести невеликі зміни та розділити налаштування для кожного виклику:
...
{% hint style="warning" %}
Знову ж таки нагадаю, якщо ваш плагін не повинен щось повертати за вашим задумом — повертайте `this`. Ланцюжки викликів у jQuery це частина магії, не варто її руйнувати. Методи `each()` та `click()` повертають об'єкт jQuery.
{% endhint %}
...
Для розуміння цього шматочка коду вам треба розібратися лише зі змінною `arguments` та методом `apply()`. Тут їм цілі статті присвятили, тож вперед:

* [https://uk.javascript.info/arguments-pseudoarray](https://uk.javascript.info/rest-parameters-spread)
* [https://uk.javascript.info/call-apply](https://uk.javascript.info/call-apply-decorators)

## Про обробники подій <a href="#event-handlers" id="event-handlers"></a>

Якщо ваш плагін «вішає» який-небудь обробник, то краще (читайте – завжди) цей обробник «вішати» у своєму власному просторі namespace:
...
Цей фінт дозволить у будь-який момент прибрати всі ваші обробники або викликати тільки ваш, що дуже зручно:
