...
</code></pre></td><td>вставляє контент після кожного елемента з вибірки, тобто якщо ти зустрічаєш рядок <code>$('p').after('&#x3C;hr/>')</code>, читай його як «<em>після</em> кожного параграфа буде додано лінію»</td></tr><tr><td><pre class="language-javascript"><code class="lang-javascript">insertAfter(element)
</code></pre></td><td>вставляє елементи з вибірки після кожного елемента переданого як аргумент, тобто якщо ти зустрічаєш рядок <code>$('&#x3C;hr/>').insertAfter('p')</code> , читай його як «лінія буде <em>додана після</em> кожного параграфа»</td></tr></tbody></table>
...
> Я тут звісно можу будувати з себе розумника, але насправді я теж постійно підглядаю імена методів, коли працюю з DOM.

{% embed url="https://anton.shevchuk.name/book/code/dom.wrap.html" %}

***

Ось тобі домашнє завдання — напиши скрипт, який додасть наступний HTML-код на сторінку:
