...
_— Чому ж вона називається каскадною?_ — це питання я часто задаю на співбесідах претендентам. Відповіддю ж буде аналогія, бо вона непохитна як перпендикулярна жаба: уяви собі каскад водоспаду, ось ти стоїш на одній зі сходинок каскаду з чорнильницею в руках, і виливаєш її вміст у воду — вся вода нижче по каскаду буде забарвлюватись у колір чорнил. Так і в CSS: якщо ти задаєш стиль на певному рівні, він поширюється вниз по "каскаду" до елементів веб-сторінки, забарвлюючи їх у "колір" твого стилю.&#x20;

_— Навіщо мені все це?_ — працюючи з jQuery, ти маєш «на відмінно» читати правила CSS, а також вміти складати CSS-селектори для пошуку необхідних елементів на сторінці. Практично всі задачі, які ти будеш вирішувати за допомогою jQuery, починаються з пошуку необхідного елемента на сторінці, тож **знання CSS-селекторів обов'язкове**.

Але давай про все по порядку, візьмемо наступний простенький приклад цілком семантичного HTML:
...
Це приклад простого і правильного HTML з невеличким додаванням CSS. Давай розберемо селектори в наведеному CSS-коді (я навмисно не виносив CSS в окремий файл, бо так наочніше):

* `body` — ці правила будуть застосовані до тегу `<body>` та всіх його нащадків, запам'ятай: налаштування шрифтів поширюються вниз по ієрархії елементів
* `h1,h2,h3` — ми обираємо теги `<h1>`, `<h2>` та `<h3>`, і встановлюємо колір шрифту для цих тегів та їх нащадків
* `#content` — обираємо елемент з `id="content"`, налаштування відступів не поширюються на нащадків, вони будуть змінюватись тільки для цього елемента
...
> Це аж ніяк не весь список, опис же всіх CSS-селекторів можна знайти на відповідній сторінці W3C: [https://www.w3.org/TR/selectors-3/](https://www.w3.org/TR/selectors-3/)

Повертаючись до нашої аналогії з водоспадом, уяви, що на наступній сходинці хтось виллє чорнила іншого кольору, ми ж у результаті отримаємо змішування кольорів. Але це в житті, а в CSS ця зміна "перефарбує воду" на всіх наступних сходинках, адже в CSS важливий порядок, порядок і пріоритети. Давай перейдемо до суті:
...
2. на другій сходинці стилі, задані користувачем в надрах налаштувань браузера; на практиці зустрічаються рідко
3. найважливіші для нас — стилі автора сторінки, але і тут все йде по порядку
   ...
   *   потім ті, що захардкодили погані люди (не ти, ти так робити не будеш) в `style=""`:
   ...
   *   і тут з'являється мітка `!important`, і все нам ламає, бо правило з такою міткою стає важливішим навіть за inline-стилі, і все йде шкереберть:
       ...
       <p style="color: blue">
         Цей параграф все одно буде червоним!
       </p>
       ```
   *   щоб перебити такий стиль, залишається лише варіант прописувати inline-стилі з міткою `!important`:&#x20;
       ...
       ```
4. пригоди з `!important` не закінчились, якщо є користувацькі стилі з цією міткою, то тепер їх вихід
5. і навіть браузери грішать з  `!important`, і такі стилі мають практично максимальний пріоритет

{% hint style="info" %}
Є ще пріоритети у правил які використовуються для [CSS анімації](https://developer.mozilla.org/ru/docs/Web/CSS/CSS\_animations/Using\_CSS\_animations) та [CSS переходів](https://developer.mozilla.org/ru/docs/Web/CSS/CSS\_transitions/Using\_CSS\_transitions), але це вже зовсім інша історія
{% endhint %}

Якщо голова ще не болить, то я також згадаю, що при розрахунку, чиї ж правила головніші, аналізується [специфічність](https://developer.mozilla.org/ru/docs/Web/CSS/Specificity) селекторів, і тут рахується наступним чином:

* розрахунок відбувається за трьома ваговими позиціями — `[0:0:0]`
* за кожен ідентифікатор елемента (`#id`) — `[1:0:0]`
* за кожен клас (`.class`), або псевдо-клас (`:pseudo`) — `[0:1:0]`
* за кожен тег (`<a>`, `<div>`, тощо) — `[0:0:1]`
* при цьому `[1:0:0]` > `[0:x:y]` > `[0:0:x]`&#x20;
* при рівності рахунку — знову «тапки» у того, хто оголошений останнім
...
Так, розрахунок специфічності може дійсно стати нетривіальною задачею, тому ось тобі пару калькуляторів на вибір:
...
{% hint style="warning" %}
Запам'ятай, що першочергово при використанні правил CSS ключовим є рівень специфічності селектора. І тільки у випадку, коли специфічності збігаються, буде важливим порядок підключення стилів:
...
{% hint style="danger" %}
Ну і повторюся, мітка `!important` - страшна річ, використовувати слід лише в крайньому випадку, таке правило має найвищий пріоритет, хоча і не має нічого спільного зі специфічністю.
{% endhint %}
...
Ось шматочок CSS для тренування, напиши відповідний йому HTML (це питання зі співбесіди ;):
