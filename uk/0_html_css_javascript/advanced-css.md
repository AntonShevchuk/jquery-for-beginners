...
Я не наполягаю на своєму варіанті, але хотілося би, щоб ви взяли на озброєння один із багатьох стандартів форматування і завжди слідували йому.

> Коли станете матьорим front-end розробником — пізнаєте всю силу CSS-препроцесорів, а поки слухайте та запам'ятовуйте.
...
У WEB використовується колірна модель [RGB](https://www.w3.org/TR/css-color-3/#rgb-color), що йменується за англійськими назвами основних колірних каналів — red, green, blue — яка спирається на цифрове позначення для будь-яких відтінків. Тому червоний колір можна записати не тільки як «red», але й ще декількома способами:
...
> Тепер ви без запинки маєте назвати кольори `#f00`, `#0f0`, `#00f`, а ті, у кого з малювання було «відмінно», назвуть і `#ff0`, `#0ff` та `#f0f` ;)
...
> Та хто тим HSL користується? ~~Не морочте собі голову!~~ HSL стає все більш популярним, і ви маєте про нього знати, і навіть розуміти, як він працює!
...
Можливо, ви ще не знаєте, але HTML-теги поділяються на блокові (block-тип) та рядкові (inline-тип). Блоковими елементами називають ті, які відображаються як прямокутник, займають усю доступну ширину всередині елемента-батька, і їх висота визначається їх змістом. Блокові теги за замовчуванням починаються і закінчуються новим рядком — це `<div>`, `<h1>` та побратими, `<p>` та інші.

Якщо хочете, щоб ваш HTML залишався валідним, слідкуйте за тим, щоб блокові елементи не розташовувались усередині рядкових елементів. Усередині рядкових тегів може бути або текст, або інші рядкові елементи.
...
Ще хотів окремо зупинитися на обчисленні ширини та висоти блокових елементів, адже тут є один нюанс. За замовчуванням висота та ширина елементів рахуються без товщини границь та внутрішніх відступів, тобто якось так:
...
Я б хотів ще розповісти про CSS-властивість `float`, але боюся, розповідь буде довгою і виснажливою. Коротенько: якщо ви вказуєте елементу властивість `float`, то:
...
Тут головне розуміти, що відбувається і вміти керувати, якщо, звісно, ви хочете хоч трішки навчитись верстати :)
...
* `absolute` — елемент розташовується відносно найближчого позиціонованого предка (не `static`), згідно з указаними координатами
