...
Хотів би одразу звернути увагу на форматування JavaScript-коду. Мій досвід мені підказує — найкраще спроєктувати стандарти форматування основної мови розробки на прикладну — JavaScript, а якщо ви хочете чогось із глобального та загальноприйнятого, то я для вас вже погуглив:
...
> На додачу поділюся невеличкою порадою: всі змінні, що містять об'єкт jQuery, найкраще іменувати, починаючи з символу «$». Повірте, така невеличка хитрість економить багато часу.
...
Ось вам наступне завдання — вгадайте значення `x` та `y` в наступному прикладі:
...
{% hint style="info" %}
Насправді `length` повертає індекс останнього елемента масиву+1, тож не потраптеся на наступну «помилку новачка»:
{% endhint %}
...
З класами, конструкторами тощо в JavaScript складніше буде, хоча для розуміння не так вже й багато треба. Запам'ятовуйте: будь-яка функція, викликана з використанням ключового слова `new`, повертає нам об'єкт, а сама стає конструктором цього об'єкта:
...
Що ж тут відбувається? Функція, оголошена всередині іншої функції, має доступ до змінних батьківської функції. Повтикайте у код, поки вас не осінить, про що я тут товкую.
...
Вступна по JavaScript затягнулась, краще не полінуйтеся, і вивчіть весь [підручник від Іллі Кантора](https://uk.javascript.info/).
