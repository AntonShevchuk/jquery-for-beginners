...
Ну и немного про содержимое элементов, для работы с ним у нас есть 2 метода: `html()` и `text()`, между ними одна существенная разница — первый умеет получать и вставлять HTML код, второй же всегда оперирует текстом. Даже если вы попытаетесь скормить ему HTML, в результате получите текст, где тэги будут приведены к [HTML entities](https://ru.wikipedia.org/wiki/%D0%9C%D0%BD%D0%B5%D0%BC%D0%BE%D0%BD%D0%B8%D0%BA%D0%B8_%D0%B2_HTML):
